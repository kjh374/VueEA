<template>
  <div class="container">
    ...
    <div class="btn-area">
      <button v-if="isEditable" @click="openForm" class="btn-primary">수정</button>
      <button v-if="isEditable" @click="submitCancel" class="btn-delete">삭제</button>
        ...
        <tr>
          <td>
            <ul class="approver-list">
              <li v-for="(approver, index) in approvers" :key="approver.approverId">
...
export default {
  ...
  computed: {
    ...
    substituteText() {
      return this.substitute && this.substitute.empNm
        ? this.substitute.pstnNm + ' ' + this.substitute.empNm
        : '없음';
    },
    contactsText() {
